...
export const HANDBOOK_TOPICS: HandbookTopic[] = [
  ...
  {
    id: 'uav_hlfc_suction',
    title: 'Гибридное Управление Ламинарным Обтеканием Крыла (HLFC) & Микроотсос Погранслоя',
    category: 'uav',
    categoryLabel: 'HLFC & Управление Погранслоем',
    icon: Wind,
    badge: 'HLFC Ламинаризация -42%',
    summary:
      'Активная задержка ламинарно-турбулентного перехода с помощью микроперфорированного отсоса пограничного слоя (HLFC) и электрогидродинамических DBD плазменных актуаторов. Снижение вязкостного трения крыла на 35–45% и расширение диапазона критических углов атаки.',
    purpose:
      'Увеличение продолжительности полета (Loiter Time) и радиуса действия БПЛА большой дальности (HALE/MALE) за счет удержания ламинарного обтекания до 70–75% хорды, подавления неустойчивости Толлмина-Шлихтинга и устранения срывных вихрей на режимах маневрирования.',
    engineeringWorkflow: {
      title: 'Инженерный Протокол Ламинаризации Пограничного Слоя HLFC',
      goal: 'Добиться максимального снижения профильного сопротивления крыла за счет задержки ламинарного перехода и предотвращения срыва потока.',
      steps: [
        {
          stepNumber: 1,
          title: 'Определение зоны ламинарной неустойчивости по методу e^N',
          action: 'Рассчитайте распределение градиента давления $dp/dx$ и определите координату $x_{\\text{crit}}$, где $N$-фактор усиления волн Толлмина-Шлихтинга достигает $N=9$.',
          uiTarget: 'Модуль «HLFC Микроотсос» -> График $N$-фактора неустойчивости',
          expectedResult: 'Локализация точки естественного перехода в турбулентность без активного воздействия.',
        },
        {
          stepNumber: 2,
          title: 'Настройка коэффициента расхода отсоса C_q и пористости обшивки',
          action: 'Задайте безразмерный коэффициент скорости отсоса $C_q = v_w / U_\\infty$ в диапазоне $(0.5 \\dots 2.5) \\times 10^{-3}$ через лазерную титановую микроперфорацию с диаметром пор $d_{\\text{pore}} = 40..60\\,\\mu\\text{m}$.',
          uiTarget: 'Слайдер «Коэффициент расхода $C_q$» и селектор зоны отсоса $x/c$',
          expectedResult: 'Уплощение профиля скорости Блазиуса и сдвиг координаты перехода $x_{\\text{tr}}$ с $28\\%$ до $72\\%$ хорды.',
        },
        {
          stepNumber: 3,
          title: 'Активация DBD плазменных актуаторов для задержки срыва',
          action: 'На околосрывных углах атаки $\\alpha > 14^\\circ$ включите диэлектрический барьерный разряд (DBD) переменного тока $V_{\\text{pp}} = 6..12\\,\\text{кВ}, f = 3..10\\,\\text{кГц}$ для генерации пристеночной струи и подпитки импульса.',
          uiTarget: 'Панель «DBD Plasma Actuator» -> Тумблер активации и слайдер напряжения',
          expectedResult: 'Увеличение критического угла атаки $\\Delta \\alpha_{\\text{stall}} = +4.5^\\circ$ и прирост несущей способности $\\Delta C_{L,\\max} = +0.35$.',
        },
        {
          stepNumber: 4,
          title: 'Оценка энергетического баланса системы отсоса',
          action: 'Сопоставьте аэродинамическую экономию мощности тяги $\\Delta P_{\\text{aero}} = \\Delta C_D \\cdot q_\\infty S V_\\infty$ с затратами электроэнергии вакуумных компрессоров $P_{\\text{pump}} = \\frac{\\dot{m} \\Delta p}{\\rho \\eta_{\\text{pump}}}$.',
          uiTarget: 'Табло «Чистый КПД HLFC & Энергетический Баланс Net Power»',
          expectedResult: 'Положительный чистый баланс экономии энергии $\\Delta P_{\\text{net}} > +18\\%$.',
        },
      ],
      pitfallsAndTroubleshooting: [
        {
          issue: 'Засорение микропор пылью, солевым аэрозолем или насекомыми (Bugs Contamination)',
          resolution: 'Применяйте импульсную продувку обратным давлением (Back-Purge Pulsing) и супергидрофобное нанопокрытие передней кромки.',
        },
        {
          issue: 'Избыточный отсос ($C_q > 4 \\times 10^{-3}$) приводит к росту энергозатрат насоса и струйной неустойчивости',
          resolution: 'Оптимизируйте $C_q$ по критерию минимума суммарного эквивалентного сопротивления $C_{D,\\text{total}} = C_D + C_{D,\\text{pump}}$.',
        },
      ],
      bestPractices: [
        'Для крыльев стреловидностью $\\chi > 20^\\circ$ обязательно комбинируйте щелевой отсос на передней кромке для подавления вихрей вторичного течения (Crossflow Instability).',
        'Используйте переменный шаг перфорации вдоль размаха крыла с концентрацией в зонах пикового неблагоприятного градиента давления.',
      ],
    },
    uiWalkthrough: {
      title: 'Интерфейс HLFC & Управления Пограничным Слоем',
      description: 'Комплексный инструмент моделирования активной ламинаризации, микроперфорации, плазменных актуаторов DBD и оценки чистого выигрыша аэрокачества.',
      controls: [
        { name: 'Коэффициент отсоса $C_q$', type: 'Slider ($0 \\dots 4 \\times 10^{-3}$)', role: 'Интенсивность микроотсоса погранслоя через пористую панель.' },
        { name: 'Зона перфорации $x_{\\text{start}} - x_{\\text{end}}$', type: 'Range Selector (% хорды)', role: 'Ограничение области размещения микропор на верхней и нижней поверхностях.' },
        { name: 'Напряжение DBD актуатора $V_{\\text{pp}}$', type: 'Slider (0 – 15 кВ)', role: 'Амплитуда высоковольтного ВЧ-разряда диэлектрического актуатора.' },
        { name: 'Частота импульсов $f_{\\text{burst}}$', type: 'Slider (100 – 2500 Гц)', role: 'Частота модуляции плазменных синтетических струй.' },
      ],
      readouts: [
        { name: 'Снижение трения $\\Delta C_f$', unit: '%', interpretation: 'Процент снижения поверхностного трения относительно полностью турбулентного профиля.' },
        { name: 'Точка перехода $x_{\\text{tr}} / c$', unit: '%', interpretation: 'Координата ламинарно-турбулентного перехода вдоль хорды крыла.' },
        { name: 'Чистый выигрыш мощности $\\Delta P_{\\text{net}}$', unit: 'Вт / %', interpretation: 'Разница между сэкономленной мощностью тяги и затратами вакуумных насосов.' },
        { name: 'Прирост аэрокачества $\\Delta (L/D)$', unit: 'ед.', interpretation: 'Увеличение аэродинамического качества планера БПЛА.' },
      ],
    },
    mathematics: {
      governingEquationLatex: 'C_q = \\frac{v_w}{U_\\infty}, \\quad \\frac{d\\theta}{dx} + \\frac{\\theta}{U_\\infty} \\frac{dU_\\infty}{dx} (2 + H) - \\frac{v_w}{U_\\infty} = \\frac{C_f}{2}',
      description: 'Интегральное уравнение импульсов фон Кармана с учетом скорости отсоса на проницаемой стенке $v_w < 0$ и метод $e^N$ Ван Ингена.',
      derivationSteps: [
        'Асимптотический ламинарный профиль скорости с однородным отсосом: $u(y) = U_\\infty (1 - e^{-|v_w| y / \\nu})$, толщина вытеснения $\\delta^* = \\nu / |v_w|$.',
        'Критическое число Рейнольдса устойчивости возрастает экспоненциально: $Re_{\\delta^*,\\text{crit}} = \\frac{U_\\infty \\delta^*}{\\nu} = \\frac{U_\\infty}{|v_w|} = \\frac{1}{C_q}$. При $C_q = 1.43 \\times 10^{-4}$ граница устойчивости $Re_{x,\\text{crit}} \\to \\infty$.',
        'Снижение коэффициента трения Блазиуса: $C_f^{\\text{lam}} = \\frac{1.328}{\\sqrt{Re_x}} \\ll C_f^{\\text{turb}} = \\frac{0.074}{Re_x^{1/5}}$.',
      ],
    },
    physicalSignificance: [
      'Ламинаризация крыла БПЛА снижает путевой расход топлива и энергии на 25–40%, что делает возможным беспосадочные полеты продолжительностью свыше 48 часов.',
      'Плазменные актуаторы DBD работают без подвижных механических элементов с временем отклика $< 1\\,\\text{мс}$, парируя резкие порывы ветра и турбулентные спутные следы.',
    ],
    references: [
      { authors: 'Joslin, R. D.', year: '1998', title: 'Aircraft Laminar Flow Control', publisher: 'Annual Review of Fluid Mechanics' },
      { authors: 'Schlichting, H., & Gersten, K.', year: '2016', title: 'Boundary-Layer Theory (9th ed.)', publisher: 'Springer-Verlag' },
      { authors: 'Corke, T. C., Enloe, C. L., & Wilkinson, S. P.', year: '2010', title: 'Dielectric Barrier Discharge Plasma Actuators for Flow Control', publisher: 'Annual Review of Fluid Mechanics' },
    ],
  },
  {
    id: 'uav_mhd_plasma',
    title: 'Магнитогидродинамические (МГД) & Криогенные Водородные Движители БПЛА',
    category: 'uav',
    categoryLabel: 'МГД, Водород & Лазер',
    icon: Atom,
    badge: 'МГД Тяга Без Винтов',
    summary:
      'Твердотельные плазменные движители на основе силы Лоренца в скрещенных электромагнитных полях ($F = J \\times B$), криогенные водородные энергоустановки (LH₂ PEMFC) с плотностью $33.3\\,\\text{кВт}\\cdot\\text{ч/кг}$ и беспроводная лазерная подпитка фотоэлементов в полете.',
    purpose:
      'Обеспечение сверхдлительного беспосадочного барражирования (120+ часов) и создание бесшумных кинематических платформ нового поколения без открытых вращающихся винтов, механических редукторов и акустического следа.',
    engineeringWorkflow: {
      title: 'Инженерный Протокол МГД Плазменного Ускорения & Водородного Энергобаланса',
      goal: 'Смоделировать твердотельное электромагнитное ускорение воздушного потока и рассчитать параметры криогенного водородного топливного стека.',
      steps: [
        {
          stepNumber: 1,
          title: 'Ионизация воздушного канала и генерация плазмы',
          action: 'Сгенерируйте ионизированный канал с проводимостью плазмы $\\sigma = 10..150\\,\\text{См/м}$ с помощью ВЧ-разряда или диодной лазерной накачки.',
          uiTarget: 'Модуль «МГД Плазма» -> Панель ионизации и генератора разряда',
          expectedResult: 'Формирование стабильного проводящего плазменного слоя в рабочем канале ускорителя.',
        },
        {
          stepNumber: 2,
          title: 'Приложение скрещенных полей E и B для создания силы Лоренца',
          action: 'Задайте индукцию постоянного магнитного поля неодимовых магнитов / ВТСП катушек $B = 0.5..2.5\\,\\text{Тл}$ и напряженность электрического поля $E = 20..100\\,\\text{кВ/м}$.',
          uiTarget: 'Слайдеры «Магнитная индукция $B$» и «Напряжение электродов $V$»',
          expectedResult: 'Возникновение плотности тока $\\mathbf{J} = \\sigma (\\mathbf{E} + \\mathbf{v} \\times \\mathbf{B})$ и объемной электромагнитной тяги $\\mathbf{F} = \\mathbf{J} \\times \\mathbf{B}$.',
        },
        {
          stepNumber: 3,
          title: 'Конфигурация криогенного бака LH₂ и стека топливных элементов PEMFC',
          action: 'Установите объем вакуумно-изолированного криобака жидкого водорода (LH₂ при $T = 20.3\\,\\text{К}$) и настройте удельную мощность топливных элементов Proton-Exchange Membrane.',
          uiTarget: 'Вкладка «Водородная Энергоустановка PEMFC» -> Слайдер массы топлива',
          expectedResult: 'Расчет запаса хода и времени беспосадочного висения: $t_{\\text{flight}} = \\frac{m_{\\text{fuel}} \\cdot LHV \\cdot \\eta_{\\text{sys}}}{P_{\\text{cruise}}} > 120\\,\\text{часов}$.',
        },
        {
          stepNumber: 4,
          title: 'Наведение лазерного луча подзарядки (Power Beaming)',
          action: 'Активируйте систему прецизионного оптического слежения FSM (Fast Steering Mirror) на длине волны $1064\\,\\text{нм}$ для дистанционной подпитки GaAs фотоэлектрических преобразователей БПЛА.',
          uiTarget: 'Панель «Лазерная передача мощности» -> Кнопка захвата цели',
          expectedResult: 'Удержание светового пятна на приемной матрице с эффективностью перехвата $\\eta_{\\text{beam}} > 92\\%$.',
        },
      ],
      pitfallsAndTroubleshooting: [
        {
          issue: 'Эффект Холла (Hall Parameter $\\beta_H = \\omega_e \\tau_e > 1$) вызывает отклонение вектора тока и снижение тяги',
          resolution: 'Используйте секционированные электроды с индивидуальной регулировкой потенциала для компенсации холловского градиента.',
        },
        {
          issue: 'Кипение и испарение жидкого водорода (Boil-off Rate) в криобаке при длительной стоянке',
          resolution: 'Применяйте многослойную экранно-вакуумную изоляцию (MLI) и активное охлаждение на микрокриокулерах Стирлинга.',
        },
      ],
      bestPractices: [
        'Комбинируйте МГД ускорители с диффузорными кольцевыми насадками для эжекции пассивного воздуха и увеличения тяговой отдачи на ватт мощности.',
        'Для лазерного питания используйте адаптивную оптику с волновым датчиком Шака-Гартмана для компенсации атмосферной турбулентности $C_n^2$.',
      ],
    },
    uiWalkthrough: {
      title: 'Интерфейс Передовых Энергетических и Движительных Систем',
      description: 'Моделирование магнитогидродинамической плазменной тяги, водородных топливных элементов и лазерной дистанционной накачки.',
      controls: [
        { name: 'Магнитная индукция $B$', type: 'Slider (0.1 – 3.0 Тл)', role: 'Напряженность поля магнитной системы ускорителя.' },
        { name: 'Плотность тока $\\mathbf{J}$', type: 'Slider ($10^2 \\dots 10^5$ А/м²)', role: 'Электрический ток в плазменном канале разряда.' },
        { name: 'Масса криогенного LH₂', type: 'Slider (0.5 – 50 кг)', role: 'Запас жидкого водорода в бортовом криостате.' },
        { name: 'Мощность лазерного излучателя $P_{\\text{laser}}$', type: 'Slider (1 – 50 кВт)', role: 'Мощность наземного волоконного лазера 1064 нм.' },
      ],
      readouts: [
        { name: 'Тяга Лоренца $F_{\\text{MHD}}$', unit: 'Н', interpretation: 'Результирующая твердотельная электромагнитная тяга плазменного канала.' },
        { name: 'Тяговая отдача $F/P$', unit: 'мН / Вт', interpretation: 'Энергетическая эффективность преобразования электричества в импульс струи.' },
        { name: 'Продолжительность полета $t_{\\text{flight}}$', unit: 'ч / сут', interpretation: 'Автономность БПЛА на водороде и лазерной подпитке.' },
        { name: 'Удельная энергоемкость $E_{\\text{spec}}$', unit: 'Вт·ч/кг', interpretation: 'Энергетическая плотность гибридной энергосистемы.' },
      ],
    },
    mathematics: {
      governingEquationLatex: '\\mathbf{F}_{\\text{Lorentz}} = \\int_V (\\mathbf{J} \\times \\mathbf{B}) dV = \\sigma (\\mathbf{E} - \\mathbf{v} \\times \\mathbf{B}) \\times \\mathbf{B} \\cdot V, \\quad P_{\\text{elec}}^{\\text{PV}} = P_{\\text{laser}} e^{-\\alpha_{\\text{atm}} R} \\eta_{\\text{GaAs}}',
      description: 'Уравнения магнитной гидродинамики (МГД) для силы Лоренца в скрещенных полях и закон Бера-Ламберта-Бугера для лазерной передачи энергии.',
      derivationSteps: [
        'Уравнение обобщенного закона Ома для ионизированного газа: $\\mathbf{J} = \\sigma (\\mathbf{E} + \\mathbf{v} \\times \\mathbf{B}) - \\frac{\\beta_H}{B} (\\mathbf{J} \\times \\mathbf{B})$.',
        'Удельный импульс МГД струи: $I_{\\text{sp}} = \\frac{u_e}{g_0} = \\frac{\\sqrt{2 \\Delta p_{\\text{MHD}} / \\rho}}{g_0}$, где $\\Delta p_{\\text{MHD}} = \\int J_y B_z dx$.',
        'Энергетический баланс криогенного водородного элемента: $W_{\\text{LH2}} = m_{\\text{H2}} \\cdot 120\\,\\text{МДж/кг} \\cdot \\eta_{\\text{PEMFC}} \\cdot \\eta_{\\text{inv}} \\cdot \\eta_{\\text{motor}}$.',
      ],
    },
    physicalSignificance: [
      'МГД движители работают без подвижных механических элементов, исключая вибрацию, трение подшипников и обеспечивая 100% акустическую скрытность.',
      'Криогенный водород обеспечивает удельную энергоемкость в 15 раз выше лучших литий-ионных аккумуляторов, открывая эру недельных беспосадочных миссий.',
    ],
    references: [
      { authors: 'Rosa, R. J.', year: '1987', title: 'Magnetohydrodynamic Energy Conversion', publisher: 'Hemisphere Publishing' },
      { authors: 'Larminie, J., & Dicks, A.', year: '2003', title: 'Fuel Cell Systems Explained (2nd ed.)', publisher: 'John Wiley & Sons' },
      { authors: 'Summerer, L., & Purcell, O.', year: '2009', title: 'Concepts for Wireless Energy Transmission via Laser', publisher: 'ESA Advanced Concepts Team' },
    ],
  },
  {
    id: 'uav_acoustic_cloaking',
    title: 'Акустическая Маскировка (ANC), Бортовой SAR Радар & Квантовая Навигация',
    category: 'uav',
    categoryLabel: 'ANC Стелс & SAR Радар',
    icon: VolumeX,
    badge: 'ANC Стелс & SAR InSAR',
    summary:
      'Комплекс малозаметности и автономности БПЛА: активное подавление шума винтов противофазной деструктивной интерференцией (ANC $\\Delta\\phi = \\pi$), миниатюрная РЛС с синтезированной апертурой (SAR/InSAR) и квантовая гравиметрическая навигация без спутников GNSS.',
    purpose:
      'Скрытие БПЛА от наземных акустических пеленгаторов (-88% дистанции обнаружения), всепогодное радиолокационное картографирование сквозь облака и туман с субметровым разрешением и неуязвимость к средствам РЭБ.',
    engineeringWorkflow: {
      title: 'Инженерный Протокол Акустического Стелса, SAR и Квантовых Сенсоров',
      goal: 'Подавить акустическую заметность винтов противофазным сигналом, синтезировать радарную карту высокого разрешения и привязать траекторию к гравитационным аномалиям.',
      steps: [
        {
          stepNumber: 1,
          title: 'Анализ спектра шума лопастных гармоник (BPF)',
          action: 'Выделите фундаментальную частоту следования лопастей $f_{\\text{BPF}} = B \\cdot \\frac{\\text{RPM}}{60}$ и высшие гармоники $m \\cdot f_{\\text{BPF}}$.',
          uiTarget: 'Модуль «Акустический Стелс ANC» -> Спектроанализатор FFT',
          expectedResult: 'Определение пиковых тональных дискрет шума вращения пропеллеров.',
        },
        {
          stepNumber: 2,
          title: 'Синтез противофазного акустического поля FxLMS',
          action: 'Включите адаптивный фильтр FxLMS (Filtered-X Least Mean Squares) для излучения акустического анти-сигнала со сдвигом фазы $\\Delta\\phi = 180^\\circ$ через кольцевые динамики.',
          uiTarget: 'Тумблер «Active Noise Cancellation (ANC)» и настройка задержки $\\tau$',
          expectedResult: 'Деструктивная интерференция, снижение уровня звукового давления на $\\Delta SPL = 16..22\\,\\text{дБА}$ и сжатие акустического купола заметности.',
        },
        {
          stepNumber: 3,
          title: 'Синтезирование апертуры бортовой FMCW SAR РЛС',
          action: 'Настройте полосу ЛЧМ модуляции $B_{\\text{chirp}} = 1.2\\,\\text{ГГц}$ в Ka-диапазоне ($24\\,\\text{ГГц}$) и скорость полета для формирования синтезированной антенной решетки.',
          uiTarget: 'Вкладка «SAR/InSAR Радар» -> Слайдер полосы $B$ и угла визирования $\\theta_{\\text{squint}}$',
          expectedResult: 'Построение 2D радиолокационного снимка высокого разрешения $\\delta r_{\\text{range}} = c / (2B) = 0.12\\,\\text{м}$ и интерферограммы высот InSAR.',
        },
        {
          stepNumber: 4,
          title: 'Квантовая гравиметрическая привязка (GNSS-Denied Navigation)',
          action: 'Активируйте квантовый атомный градиентометр рубидия ($^{87}\\text{Rb}$) для непрерывного измерения аномалий силы тяжести $\\Delta g$ и сопоставления с цифровой картой геоида.',
          uiTarget: 'Панель «Квантовая Навигация» -> Кнопка старта гравитационного сопоставления TRN',
          expectedResult: 'Дрейф координат $< 2.5\\,\\text{м}$ за 24 часа автономного полета без использования GPS/ГЛОНАСС.',
        },
      ],
      pitfallsAndTroubleshooting: [
        {
          issue: 'Фазовое рассогласование ANC $\\delta \\phi > 15^\\circ$ из-за температурного дрейфа скорости звука $c(T)$',
          resolution: 'Внедрите непрерывную температурную компенсацию в ядро DSP: $c = \\sqrt{\\gamma R T_{\\text{amb}}}$.',
        },
        {
          issue: 'Спекл-шум (Speckle Noise) и геометрические искажения Layover на SAR снимках горной местности',
          resolution: 'Используйте многоракурсную фильтрацию Ли-Фроста (Multi-Look Lee Filter) и ортотрансформирование по карте DEM.',
        },
      ],
      bestPractices: [
        'Комбинируйте активный ANC с пассивными совиными шевронами на задних кромках лопастей: пассивные зубья дробят широкополосный турбулентный шум, а ANC гасит тональные пики BPF.',
        'Для InSAR интерферометрии строго контролируйте стабильность базовой линии между двумя антеннами с помощью лазерного интерферометра перемещений.',
      ],
    },
    uiWalkthrough: {
      title: 'Интерфейс Акустической Маскировки, SAR и Квантовых Сенсоров',
      description: 'Управление активным шумоподавлением, синтезированием апертуры радиолокатора и квантовым гравиметрическим счислением пути.',
      controls: [
        { name: 'Фазовый сдвиг ANC $\\Delta\\phi$', type: 'Slider ($150^\\circ \\dots 210^\\circ$)', role: 'Точная подстройка фазы антизвукового излучателя.' },
        { name: 'Полоса зондирования SAR $B$', type: 'Slider (200 – 1500 МГц)', role: 'Ширина спектра ЛЧМ импульса для дальностного разрешения.' },
        { name: 'Время накопления синтеза $T_{\\text{syn}}$', type: 'Slider (0.2 – 3.0 с)', role: 'Длина синтезированной апертуры $L_{\\text{syn}} = V \\cdot T_{\\text{syn}}$.' },
        { name: 'Чувствительность градиометра', type: 'Slider ($10^{-9} \\dots 10^{-12}$ g)', role: 'Точность квантовой атомной интерферометрии холодных атомов.' },
      ],
      readouts: [
        { name: 'Эффективность ANC $\\Delta SPL$', unit: 'дБА', interpretation: 'Величина снижения уровня шума дрона в точке наблюдения.' },
        { name: 'Дальность обнаружения $r_{\\text{detect}}$', unit: 'м', interpretation: 'Дистанция, на которой акустический пеленгатор может обнаружить аппарат.' },
        { name: 'Разрешение SAR $(\\delta r, \\delta a)$', unit: 'см', interpretation: 'Пространственное разрешение РЛС по дальности и азимуту.' },
        { name: 'Круговое вероятное отклонение CEP', unit: 'м', interpretation: 'Точность квантовой автономной навигации без спутниковой связи.' },
      ],
    },
    mathematics: {
      governingEquationLatex: '\\Delta SPL_{\\text{ANC}} = -10 \\log_{10}\\left(1 + A^2 - 2A\\cos(\\Delta\\phi)\\right), \\quad \\delta r_{\\text{range}} = \\frac{c}{2 B}, \\quad \\delta r_{\\text{az}} = \\frac{L_a}{2}',
      description: 'Уравнение интерференции двух когерентных акустических волн и фундаментальные соотношения разрешения РЛС с синтезированной апертурой (SAR).',
      derivationSteps: [
        'Суммарное звуковое давление: $p_{\\text{tot}}(t) = p_0 \\sin(\\omega t) + A p_0 \\sin(\\omega t + \\Delta\\phi)$. При $A=1, \\Delta\\phi=\\pi \\implies p_{\\text{tot}} \\equiv 0$.',
        'Дистанция акустического обнаружения по закону сферического расхождения: $r_{\\text{detect}} = r_0 \\cdot 10^{\\frac{SPL(r_0) - L_{\\text{bkg}} - \\Delta SPL_{\\text{ANC}}}{20}}$. При $\\Delta SPL = 18\\,\\text{дБА} \\implies r_{\\text{detect}} \\to 0.125 r_0$ (-87.5%).',
        'Азимутальное разрешение SAR не зависит от расстояния до цели: $\\delta r_{\\text{az}} = \\frac{\\lambda R}{2 L_{\\text{syn}}} = \\frac{\\lambda R}{2 (\\lambda R / L_a)} = \\frac{L_a}{2}$.',
      ],
    },
    physicalSignificance: [
      'Активный акустический стелс превращает жужжащий дрон в бесшумную тень, позволяя вести скрытное наблюдение с малых высот (30–50 м).',
      'Миниатюрный SAR радар видит сквозь облака, дым, туман и камуфляжные сетки днем и ночью, формируя 3D цифровую модель рельефа.',
      'Квантовая гравиметрия гарантирует 100% неуязвимость к радиоэлектронному подавлению (Anti-Jamming & Anti-Spoofing).',
    ],
    references: [
      { authors: 'Nelson, P. A., & Elliott, S. J.', year: '1992', title: 'Active Control of Sound', publisher: 'Academic Press' },
      { authors: 'Cumming, I. G., & Wong, F. H.', year: '2005', title: 'Digital Processing of Synthetic Aperture Radar Data', publisher: 'Artech House' },
      { authors: 'Peters, A., Chung, K. Y., & Chu, S.', year: '2001', title: 'High-Precision Gravity Measurements Using Atom Interferometry', publisher: 'Metrologia' },
    ],
  },
];
...
export const PARAMETER_DECODER_ITEMS: ParameterDecoderItem[] = [
  ...
  {
    id: 'uav_hlfc_cq',
    name: 'Коэффициент микроотсоса HLFC',
    symbolLatex: 'C_q = \\frac{v_w}{U_\\infty}',
    category: 'uav_cad',
    categoryName: 'БПЛА & Ламинаризация',
    location: 'БПЛА -> Гибридный Микроотсос HLFC (Слайдер)',
    meaning: 'Безразмерное отношение скорости отсасывания пограничного слоя через микроперфорированную обшивку к скорости набегающего потока.',
    howToConfigure: 'Оптимальное значение $(0.8 \\dots 2.2) \\times 10^{-3}$. Сдвигает ламинарный переход до 70–75% хорды крыла.',
    howToObtain: 'Задается в модуле HLFC $\\to$ снижает суммарное профильное трение на 35–45%.',
    badge: 'HLFC Отсос',
  },
  {
    id: 'uav_mhd_lorentz',
    name: 'Сила Лоренца МГД ускорителя',
    symbolLatex: '\\mathbf{F}_{\\text{Lorentz}} = \\int (\\mathbf{J} \\times \\mathbf{B}) dV',
    category: 'uav_cad',
    categoryName: 'БПЛА & Движители',
    location: 'БПЛА -> МГД Плазменный Двигатель (Табло)',
    meaning: 'Объемная электромагнитная сила тяги, возникающая при пропускании тока через ионизированный плазменный канал в поперечном магнитном поле.',
    howToConfigure: 'Увеличивайте магнитную индукцию $B$ (ВТСП катушки) и плотность плазменного тока $\\mathbf{J}$ для повышения тяги.',
    howToObtain: 'Обеспечивает твердотельное ускорение воздуха без винтов, редукторов и механического износа.',
    badge: 'МГД Плазма',
  },
  {
    id: 'uav_anc_delta_spl',
    name: 'Эффективность шумоподавления ANC',
    symbolLatex: '\\Delta SPL = -10 \\log_{10}(2 - 2\\cos\\Delta\\phi)',
    category: 'uav_cad',
    categoryName: 'БПЛА & Акустика',
    location: 'БПЛА -> Акустическая Маскировка ANC (Табло)',
    meaning: 'Величина снижения уровня звукового давления гармоник вращения винтов (BPF) за счет генерации противофазной акустической волны.',
    howToConfigure: 'Поддерживайте фазовую точность адаптивного алгоритма FxLMS $\\Delta\\phi \\approx 180^\\circ$ для достижения $\\Delta SPL \\ge 18$ дБА.',
    howToObtain: 'Уменьшает радиус зоны акустического обнаружения БПЛА наземными сенсорами на 88%.',
    badge: 'ANC Стелс',
  },
  {
    id: 'uav_sar_resolution',
    name: 'Разрешение SAR радара по дальности',
    symbolLatex: '\\delta r = \\frac{c}{2 B_{\\text{chirp}}}',
    category: 'uav_cad',
    categoryName: 'БПЛА & Сенсоры',
    location: 'БПЛА -> Миниатюрный SAR Радар (Табло)',
    meaning: 'Минимальное пространственное расстояние между двумя отражателями, различимыми радиолокатором с синтезированной апертурой.',
    howToConfigure: 'Для получения сверхвысокого разрешения ($< 15$ см) увеличивайте полосу ЛЧМ девиации $B$ до $1.2\\dots 1.5$ ГГц.',
    howToObtain: 'Обеспечивает всепогодную радиолокационную съемку сквозь облака, дождь, дым и листву.',
    badge: 'SAR Радар',
  },
  {
    id: 'uav_laser_power_beaming',
    name: 'Мощность лазерной подпитки',
    symbolLatex: 'P_{\\text{elec}} = P_{\\text{laser}} \\cdot e^{-\\alpha R} \\cdot \\eta_{\\text{GaAs}}',
    category: 'uav_cad',
    categoryName: 'БПЛА & Энергетика',
    location: 'БПЛА -> Лазерная Передача Энергии (Табло)',
    meaning: 'Электрическая мощность, вырабатываемая бортовыми фотопреобразователями арсенида галлия (GaAs) при облучении наземным лазером 1064 нм.',
    howToConfigure: 'Применяйте прецизионное наведение зеркалами FSM и антитурбулентную адаптивную оптику для максимизации КПД.',
    howToObtain: 'Позволяет БПЛА оставаться в воздухе неограниченно долго без посадок на перезарядку.',
    badge: 'Laser Beaming',
  },
  {
    id: 'uav_lh2_energy_density',
    name: 'Энергоемкость жидкого водорода LH₂',
    symbolLatex: 'E_{\\text{spec}} = 33.3 \\text{ кВт}\\cdot\\text{ч/кг}',
    category: 'uav_cad',
    categoryName: 'БПЛА & Энергетика',
    location: 'БПЛА -> Криогенная Водородная Энергоустановка (Индикатор)',
    meaning: 'Низшая теплотворная способность жидкого водорода (120 МДж/кг), преобразуемая в электроэнергию в топливных элементах PEMFC с КПД 55–65%.',
    howToConfigure: 'Используйте криогенные баки с вакуумной изоляцией MLI при температуре 20.3 К для снижения испарения.',
    howToObtain: 'Обеспечивает беспосадочный полет дрона в течение 120+ часов (5 суток непрерывного барражирования).',
    badge: 'LH₂ PEMFC',
  },
  {
    id: 'uav_quantum_nav_error',
    name: 'Дрейф квантовой навигации',
    symbolLatex: '\\sigma_{\\text{pos}} < 2.5 \\text{ м / 24 ч}',
    category: 'uav_cad',
    categoryName: 'БПЛА & Навигация',
    location: 'БПЛА -> Квантовая Гравиметрия (Индикатор)',
    meaning: 'Накопленная погрешность определения координат атомным градиентометром холодного рубидия-87 в условиях полного отсутствия GNSS.',
    howToConfigure: 'Автоматическая привязка к аномалиям гравитационного поля Земли (Terrain Relative Gravity Navigation).',
    howToObtain: 'Полная неуязвимость к средствам радиоэлектронного подавления и спуфинга GPS.',
    badge: 'Quantum GNSS-Denied',
  },
];
...
export const WORKSPACE_ZONES: WorkspaceZoneInfo[] = [
  ...
  },
  {
    ...
    role: 'Комплекс передовых инженерных лабораторий: расчет винтомоторных групп (BEM), навигация в условиях помех РЭБ с фильтром EKF2, радиолинии ретрансляции, VTOL-конвертопланы, роевой строй (V-Formation), аэроакустика FW-H, отказ моторов, обледенение, сверхзвуковые скачки, а также технологии прорыва: гибридная ламинаризация HLFC с микроотсосом погранслоя и DBD плазмой, твердотельные МГД движители на силе Лоренца с криогенным водородом LH₂ и лазерной подпиткой, активный акустический стелс ANC, бортовой SAR радар и квантовая навигация без GNSS.',
    keyControls: [
      'BEM Роторы: Диаметр винта, шаг, профиль лопасти, обороты RPM, расчет тяги $T$ и мощности $P$',
      'HLFC Ламинаризация & DBD: Коэффициент расхода микроотсоса $C_q$, напряжение разряда $V_{\\text{pp}}$, снижение трения до -42%',
      'МГД & LH₂ Водород: Магнитная индукция $B$, сила Лоренца $\\mathbf{J} \\times \\mathbf{B}$, топливный стек PEMFC, лазерное питание',
      'Акустический Стелс ANC & SAR: Подавление шума противофазой ($\\Delta SPL \\ge 18$ дБА), радар SAR/InSAR, квантовый градиентометр',
    ...
    inputsAndOutputs: {
      inputs: 'Геометрические, плазменные, акустические, радиочастотные и роевые параметры миссии БПЛА.',
      outputs: 'Тяговооруженность, диаграммы направленности шума, поля ламинарного обтекания, SAR радиоснимки, Парето-фронт САПР.',
    },
    solversConnected: [
      'Blade Element Momentum Theory с поправками Прандтля и Глауэрта',
      'Модель устойчивости погранслоя $e^N$ и уравнение импульсов с отсосом $v_w$',
      'МГД система уравнений Навье-Стокса с силой Лоренца $\\mathbf{J} \\times \\mathbf{B}$',
      'Адаптивный алгоритм шумоподавления FxLMS и аналогия Фокса Вильямса — Хокингса (FW-H)',
      '15-состояний Расширенный Фильтр Калмана (EKF2) + RAIM $\\chi^2$',
      'Алгоритм синтезирования апертуры Range-Doppler SAR/InSAR',
    ],
    proTip: 'Для достижения максимальной дальности полета (HALE) объединяйте микроотсос HLFC на передней кромке крыла с криогенной водородной энергоустановкой LH₂ PEMFC и активным акустическим шумоподавлением ANC.',
    colorTheme: {
...
export const SOP_RECIPES: SOPRecipe[] = [
  ...
  },
  {
    id: 'recipe_uav_hlfc',
    recipeNumber: 17,
    title: 'Рецепт 17: Настройка Гибридной Ламинаризации HLFC & DBD Плазмы Крыла БПЛА',
    tag: 'HLFC & Ламинаризация',
    targetDomain: 'БПЛА & Роторы',
    difficulty: 'Продвинутый',
    category: 'uav',
    categoryName: 'БПЛА & Аэродинамика',
    prerequisites: ['Распределение давления Cp(x)', 'Градиент dp/dx', 'Критерий e^N устойчивости'],
    verificationCriteria: 'Сдвиг ламинарного перехода x_tr > 70% хорды, снижение профильного трения Delta_Cf >= 35%, положительный чистый выигрыш Net Power > +15%.',
    relatedTopicId: 'uav_hlfc_suction',
    goal: 'Задержать турбулентный переход пограничного слоя на крыле БПЛА большой дальности с помощью микроперфорированного отсоса и диэлектрических плазменных актуаторов DBD.',
    estimatedTime: '3 минуты',
    steps: [
      {
        stepNumber: 1,
        title: 'Переход в модуль HLFC микроотсоса',
        description: 'В панели «Аэродинамика БПЛА & Спецмодули» переключитесь на вкладку «HLFC Микроотсос & DBD Плазма».',
        whereToClick: 'Вкладка «HLFC Микроотсос & DBD Плазма»',
        expectedOutcome: 'Отображение профиля пограничного слоя Блазиуса, кривой e^N фактора и схемы микроперфорации.',
      },
      {
        stepNumber: 2,
        title: 'Подбор коэффициента отсоса C_q',
        description: 'Установите слайдер коэффициента отсоса C_q в значение 1.4e-3 и задайте зону перфорации от 5% до 45% хорды.',
        whereToClick: 'Слайдер «Коэффициент расхода C_q»',
        expectedOutcome: 'Устранение точки перегиба в профиле скорости, ламинаризация течения и расчет снижения сопротивления.',
      },
      {
        stepNumber: 3,
        title: 'Активация DBD плазменного актуатора',
        description: 'Включите высоковольтный барьерный разряд (8 кВ, 5 кГц) для предотвращения отрыва на околосрывных углах атаки.',
        whereToClick: 'Тумблер «Включить DBD Плазменный Актуатор»',
        expectedOutcome: 'Генерация электрогидродинамической пристеночной струи и увеличение срывного угла атаки на +4.5°.',
      },
    ],
    proTip: 'Для минимального сопротивления оптимизируйте расход C_q: точка оптимума находится на границе, где производная экономии тяги dP_aero/dC_q равна затратам мощности насоса dP_pump/dC_q.',
  },
  {
    id: 'recipe_uav_mhd_propulsion',
    recipeNumber: 18,
    title: 'Рецепт 18: Конфигурация Твердотельного МГД Движителя & Криогенной Системы LH₂',
    tag: 'МГД Плазма & Водород',
    targetDomain: 'БПЛА & Роторы',
    difficulty: 'Экспертный',
    category: 'uav',
    categoryName: 'БПЛА & Энергетика',
    prerequisites: ['Магнитная индукция B >= 1.0 Тл', 'Проводимость плазмы sigma >= 20 См/м', 'Запас LH2 в криобаке'],
    verificationCriteria: 'Удельная сила Лоренца F_Lorentz > 15 Н/канал, КПД топливного элемента PEMFC > 58%, расчетное время полета > 120 ч.',
    relatedTopicId: 'uav_mhd_plasma',
    goal: 'Смоделировать бесшумную тягу на основе силы Лоренца в скрещенных полях и рассчитать многосуточный полет на криогенном жидком водороде с лазерной подпиткой.',
    estimatedTime: '4 минуты',
    steps: [
      {
        stepNumber: 1,
        title: 'Инициализация МГД ускорителя',
        description: 'Перейдите во вкладку «МГД Плазменный Двигатель & Криогенный LH₂».',
        whereToClick: 'Вкладка «МГД Плазменный Двигатель & Криогенный LH₂»',
        expectedOutcome: 'Загрузка 3D схемы скрещенных полей E x B, расчетного плазменного канала и теплового баланса криостата.',
      },
      {
        stepNumber: 2,
        title: 'Настройка электромагнитных полей и тяги',
        description: 'Установите индукцию ВТСП катушек B = 1.8 Тл и ток плазменного разряда J = 4.5 кА/м².',
        whereToClick: 'Слайдеры магнитной индукции B и плотности тока J',
        expectedOutcome: 'Расчет объемной силы Лоренца, импульса струи I_sp и тяговой отдачи (мН/Вт).',
      },
      {
        stepNumber: 3,
        title: 'Расчет запаса хода на LH₂ и лазере',
        description: 'Задайте массу жидкого водорода 12 кг в вакуумном криобаке и включите лазерную подпитку 1064 нм мощностью 10 кВт.',
        whereToClick: 'Панель «Криогенный LH₂ PEMFC & Лазерный Приемник»',
        expectedOutcome: 'График энергобаланса фиксирует непрерывное беспосадочное висение БПЛА на протяжении 144 часов (6 суток).',
      },
    ],
    proTip: 'Применение секционированных электродов предотвращает падение тяги из-за эффекта Холла и выравнивает распределение плотности тока по длине ускорителя.',
  },
  {
    id: 'recipe_uav_acoustic_sar',
    recipeNumber: 19,
    title: 'Рецепт 19: Активация Акустического Стелса ANC, Бортового SAR Радара & Квантовой Навигации',
    tag: 'ANC Стелс & SAR Радар',
    targetDomain: 'БПЛА & Роторы',
    difficulty: 'Экспертный',
    category: 'uav',
    categoryName: 'БПЛА & Сенсоры',
    prerequisites: ['Спектр шума винтов BPF', 'Полоса ЛЧМ SAR B >= 1.0 ГГц', 'Карта геоида гравиметрии'],
    verificationCriteria: 'Снижение шума Delta_SPL >= 18 дБА, сжатие зоны обнаружения на 88%, разрешение SAR dr < 15 см, дрейф квантовой навигации < 2.5 м.',
    relatedTopicId: 'uav_acoustic_cloaking',
    goal: 'Обеспечить акустическую скрытность дрона от наземных пеленгаторов, построить SAR радиолокационную карту сквозь облака и выполнить полет без сигналов GNSS.',
    estimatedTime: '3 минуты',
    steps: [
      {
        stepNumber: 1,
        title: 'Запуск модуля акустического стелса и SAR',
        description: 'Откройте вкладку «Акустический Стелс ANC, SAR Радар & Квантовая Навигация».',
        whereToClick: 'Вкладка «Акустический Стелс ANC, SAR Радар & Квантовая Навигация»',
        expectedOutcome: 'Отображение спектрограммы шума, 3D купола акустической заметности, интерфейса SAR и квантового градиентометра.',
      },
      {
        stepNumber: 2,
        title: 'Калибровка противофазного шумоподавления ANC',
        description: 'Включите фильтр FxLMS и выставьте фазовый сдвиг Delta_phi = 180° с температурной коррекцией скорости звука.',
        whereToClick: 'Тумблер «Включить ANC» и слайдер фазы',
        expectedOutcome: 'Снижение уровня звукового давления на 20 дБА и уменьшение дистанции обнаружения с 420 м до 50 м.',
      },
      {
        stepNumber: 3,
        title: 'Синтезирование апертуры SAR и квантовая привязка',
        description: 'Активируйте режим картографирования Stripmap SAR (полоса 1.2 ГГц) и запустите квантовую гравитационную навигацию TRN.',
        whereToClick: 'Кнопки «Синтезировать SAR Снимок» и «Старт Квантовой Навигации»',
        expectedOutcome: 'Построение радиолокационной 2D/3D карты высокого разрешения (12 см) и фиксация траектории без спутниковой связи.',
      },
    ],
    proTip: 'Комбинация зубчатых шевронов на лопастях винтов с активным ANC устраняет как широкополосный шум турбулентных вихрей, так и дискретные лопастные тона.',
  },
];
